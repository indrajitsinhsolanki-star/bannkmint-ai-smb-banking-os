...
# Pydantic models for API responses (without SQLModel inheritance)
...
def init_default_data():
    ...
    existing_accounts = execute_query("SELECT COUNT(*) as count FROM accounts")
    if existing_accounts[0]['count'] == 0:
        account_id = generate_id()
        execute_query(
            "INSERT INTO accounts (id, name, account_type, balance) VALUES (?, ?, ?, ?)",
            (account_id, "Default Business Account", "business_checking", 0.0)
        )
